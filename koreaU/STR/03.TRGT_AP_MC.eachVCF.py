...
def mk_df_out(out,mc,ap,am):
    ...
    while 1:
        ...
        if count == 1:
            header = 'ID\tCHROM\tPOS\tTRID\tMOTIFS\tSTRUC\tALT\tAllele\tMC\tAP\n' # alt "." 경우 STR가 없는 거
...
def main():
    ...
    long_out = outDir + inVCF_long.replace(".vcf.gz",".MC_AP.txt")
    #short_tmp = inVCF_short.replace(".vcf.gz",".tmp")

    long_mc = outDir + inVCF_long.replace(".vcf.gz",".tmp.mc")
    long_ap = outDir + inVCF_long.replace(".vcf.gz",".tmp.ap")
    long_am = outDir + inVCF_long.replace(".vcf.gz",".tmp.am")

    os.system("bcftools query -f '%%CHROM\\t%%POS\\t%%TRID\\t%%MOTIFS\\t%%STRUC\\t%%ALT[\\t%%SAMPLE=%%MC]\\n' %s > %s"%(inVCF_long,long_mc))
    os.system("bcftools query -f '%%CHROM\\t%%POS\\t%%TRID\\t%%MOTIFS\\t%%STRUC\\t%%ALT[\\t%%SAMPLE=%%AP]\\n' %s > %s"%(inVCF_long,long_ap))
    os.system("bcftools query -f '%%CHROM\\t%%POS\\t%%TRID\\t%%MOTIFS\\t%%STRUC\\t%%ALT[\\t%%SAMPLE=%%AM]\\n' %s > %s"%(inVCF_long,long_am))
