...
def mark_duplicated():
    ...
    shDir2 = shDir + "01-1.shortread_markDup/"
    os.system("mkdir %s"%shDir2)
    for i in dfs:
...
def bqsr():
    print("BQSR ")
    shortread_bqsr_Dir = shortread_Dir + "01-2.shortread_bqsr"
    os.system("mkdir "+shortread_bqsr_Dir)

    dfs= glob.glob(shortread_mapping + "*.dedup.bam")
    shDir2 = shDir + "01-2.shortread_bqsr/"
    os.system("mkdir %s"%shDir2)

    for df in dfs:
        shpath = df.replace(shortread_mapping,shDir2).replace(".bam",".bqsr.sh")
        out = df.replace(".bam",".bqsr").replace(shortread_mapping,"")
        with open(shpath,"w") as shout:
            shout.write("sudo docker run -v \"%s\":\"/input\" -v \"%s\":\"/output\" -v \"%s\":\"/ref\" \
                -it broadinstitute/gatk bash -c 'gatk BaseRecalibrator \
                    ...
                    -I /input/%s -O /output/%s.table'\n"%(shortread_mapping,shortread_bqsr_Dir,refDir,df.replace(shortread_mapping,""),out))
            #out = df.replace(".bam",".bqsr").replace(shortread_mapping,"")
            shout.write("sudo docker run -v \"%s\":\"/input\" -v \"%s\":\"/output\" -v \"%s\":\"/ref\" \
                -it broadinstitute/gatk bash -c 'gatk ApplyBQSR \
                    -R /ref/HLA.target.fasta \
                    -I /input/%s \
                    --bqsr-recal-file /output/%s.table \
                    -O /output/%s.bam'\n"%(shortread_mapping,shortread_bqsr_Dir,refDir,df.replace(shortread_mapping,""),out,out))
...
def deepvariant_calling_bqsr():
    print("Variant Calling using Deepvariant after BQSR")
    #shortread_VC_Dir = shortread_Dir + "02.variant.call_BQSR/"
    shortread_VC_Dir = "/DATA/smkim/HLA_seq/MHC_Illumina_Rawdata/02.variant.call_DV_BQSR/"
    os.system("mkdir "+shortread_VC_Dir)
    shortread_mapping = shortread_Dir + "01-2.shortread_bqsr/" 
    dfs= glob.glob(shortread_mapping + "*bam")
    shDir2 = shDir + "02.variant.call_DV_BQSR/"
...
def gatk_calling_bqsr():
    print("Variant Calling using GATK haplotypcall after BQSR ")
    
    #shortread_VC_Dir = shortread_Dir + "02.variant.call_BQSR/"
    #shortread_VC_Dir = "/DATA/smkim/HLA_seq/MHC_Illumina_Rawdata/"
    shortread_VC_Dir = "/DATA/smkim/HLA_seq/MHC_Illumina_Rawdata/02.variant.call_gatk_BQSR/"
    os.system("mkdir "+shortread_VC_Dir)
    shortread_mapping = shortread_Dir + "01-2.shortread_bqsr/"
    dfs= glob.glob(shortread_mapping + "*.bam")
    shDir2 = shDir + "02.variant.call_gatk_BQSR/"
    os.system("mkdir %s"%shDir2)
