...
head(ref2)
rm1 <- ref1 %>% rbind(ref2) %>% select(ID1,`바이오뱅크과 및 역학과 확인`,후속조치) %>% rename("ID" = ID1)
rm2 <- ref1 %>% rbind(ref2) %>% select(ID2,`바이오뱅크과 및 역학과 확인`,후속조치) %>% rename("ID" = ID2)a
rmall <- rbind(rm1,rm2)
rmall
head(df)
...
q %>% dim()
#writexl::write_xlsx(rmselect,"둘중하나제외리스트_rmlist.MZlistafterBioBankcheck_20221202.xlsx")

head(df)
head(ref4)
df[!df$ID %in% rmselect$ID),]

ref4 %>% select(ID1,ID2,`바이오뱅크과 및 역학과 확인`,후속조치) %>%
  left_join(df %>% select(ID,F_MISS) %>% rename("ID1" = ID),by='ID1') %>% rename("ID1_MISS" = F_MISS) %>%
