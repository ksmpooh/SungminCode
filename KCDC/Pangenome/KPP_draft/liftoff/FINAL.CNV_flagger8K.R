...
df <- read_table("merge.with_flagger.gene_protein_coding.selected.Hap90.txt",col_names=T) %>%
...
tag_has <- function(tag, key){
  tag <- replace_na(tag, "")
  str_detect(paste0(",", tag, ","), paste0(",", key, ","))
}

df_pc %>% filter(tag %in% c("confirm_experimentally","")) %>%
...
df_pc %>%
  count(ID,gene_name) %>% filter(n != 1) %>% 
  count(ID) %>% write.table("/ADATA/pangenome/liftoff/99.summary/liftoff.CNV.bysample.window8k.txt",col.names = T,row.names = F,quote = F,sep = "\t")
#write.table("/ADATA/pangenome/liftoff/99.summary/gene.annotation.byhap.txt",col.names = T,row.names = F,quote = F,sep = "\t") 
df_pc %>%
  count(ID,gene_name) %>% filter(n != 1) %>% 
  count(gene_name) %>% write.table("/ADATA/pangenome/liftoff/99.summary/liftoff.CNV.byGene.window8k.txt",col.names = T,row.names = F,quote = F,sep = "\t")
...
df_pc %>%
  ...
  write.table("/ADATA/pangenome/liftoff/99.summary/liftoff.CNV.raw.window8k.txt",col.names = T,row.names = F,quote = F,sep = "\t")


############################### new pic CNV

setwd("/Users/ksmpooh/Desktop/KCDC/pangenome/KPPD/liftoff/with_flagger_8k")

kppd_raw <- read_table("liftoff.CNV.raw.window8k.txt")
head(kppd_raw)
dim(kppd_raw)
#writexl::write_xlsx(kppd_raw,"liftoff.CNV.raw.window8k.xlsx")


kppd_bysample <- read_table("liftoff.CNV.bysample.window8k.txt")
head(kppd_bysample)
kppd_bysample %>% 
  ...
  writexl::write_xlsx("liftoff.CNV.bysample8k.forTable.xlsx")
...
kppd_bysample %>%
  ...
  annotate("text", x = 1.3, y = 33, label = "HPRC",color = "red", hjust = 0, size = 4) +
  annotate("text", x = 1.4, y = 51, label = "CPC",color = "green4", hjust = 0, size = 4) + 
...
kppd_bysample_plot %>% group_by(cohort) %>%
  summarise(mean = mean(n),median = median(n),max  = max(n), min  = min(n))
#  cohort  mean median   max   min
#1 nonT2T  58.2     57    92    31
#2 T2T     58.8     56    83    47
...
kppd_byGene <- read_table("liftoff.CNV.byGene.window8k.txt")
head(kppd_byGene)
dim(kppd_byGene)

#kppd_byGene %>% arrange(-n) %>% rename(Gene = gene_name,Frequency = n) %>%  writexl::write_xlsx("liftoff.CNV.byGene.window8k.forTable.xlsx")
...
kppd_byGene %>% filter(!(gene_name %in% hprc$gene)) %>% filter(!(gene_name %in% cpc$gene)) %>% filter(n>1) %>% #dim()
  select(gene_name) %>% write.table("liftoff.CNV.byGene.window8k.KPPDunique.notsingleton.txt",col.names = F,row.names = F,quote = F)

kppd_byGene %>% filter(!(gene_name %in% hprc$gene)) %>% filter(!(gene_name %in% cpc$gene))  %>% 
  write.table("liftoff.CNV.byGene.window8k.KPPDunique.txt",col.names = F,row.names = F,quote = F)
...
merge_freq %>% select(gene_name,KPPD,HPRC) %>%
  ...
  labs(caption = "KPP ∩ HPRC : 241 CNVs") + 
  #annotate("text",x = 1, y = 0,label = "KPP ∩ HPRC : 241 CNVs",hjust = 1, vjust = 0,size = 5,color = "black" ) +
  ylim(0,1) + xlim(0,1) + 
  theme_step1() + 
  theme(plot.caption = element_text(hjust = 0.5,size = 15,color = "black")) -> p1

library(cowplot)
merge_freq %>% select(gene_name,KPPD,CPC) %>% 
  rename(KPP = KPPD) %>%
  na.omit() %>% #dim() #477
  ggplot(aes(x=CPC,y=KPP)) + 
  ...
  ylim(0,1) + xlim(0,1) + 
  labs(caption = "KPP ∩ CPC : 477 CNVs") + 
  theme_step1() + 
  #annotate("text",x = 1, y = 0,label = "KPP ∩ CPC : 477 CNVs",hjust = 1, vjust = 0,size = 5,color = "black" ) +
  theme(axis.title.y=element_blank(),
        plot.caption = element_text(
          hjust = 0.5,size = 15,color = "black"))-> p2
