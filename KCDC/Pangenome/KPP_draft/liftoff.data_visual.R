...
df_cnv_count %>% filter(ref !="T2T") %>%
  group_by(ref) %>%  arrange(n) %>% mutate(ID_ordered = factor(ID, levels = unique(ID))) %>%  # n ?��?????��?? level ??��
  ungroup() %>%
...
venn.plot <- venn.diagram(
  ...
  cex = 2,               # ?????? ???? ǥ?? ��??
  cat.cex = 3,         # ????(label) ũ??
  #cat.pos = c(-20, 20),  # label ��ġ ����
  cat.fontface = "bold",
  scaled = TRUE,         # ??? ???? ??�� ?ݿ?
  filename = NULL        # ?????? ȭ?鿡 ?ٷ? ???? (???? ???? X)
)
...
cp_num %>% filter(coding_type == "Protein_coding_genes") %>% 
  ...
  mutate(asssembly_level = factor(asssembly_level, levels = rev(unique(asssembly_level)))) %>%  # ???? ???? ????
...
######## FINAL
setwd("~/Desktop/KCDC/pangenome/KPPD/liftoff/count/")

kppd_gene <- read_table("KPPD_GRCh38_gene.protein_coding_count.txt")
kppd_tx <- read_table("KPPD_GRCh38_transcripts.protein_coding_count.txt")
sampleinfo <- read_table("~/Desktop/KCDC/pangenome/00.datacheck/KPP_draft_assembly_matchingID.with.Gender.20250224.txt")
head(sampleinfo)
...
kppd_gene %>% rbind(kppd_tx) %>% #head()
  mutate(
    total = case_when(
      coding_type == "Protein_coding_genes"  ~ 20089,
      coding_type == "Noncoding_genes"       ~ 57606,
      coding_type == "Protein_coding_trans"  ~ 116490,
      coding_type == "Noncoding_trans"       ~ 271454,
      TRUE ~ NA_real_
    ),
    annotation_pct = (n / total) * 100
  ) %>% 
  mutate(sampleID = str_split_fixed(ID,"_",2)[,1]) %>%
  mutate(cohort = ifelse(sampleID %in% c("KPPD132","KPPD131","KPPD130","KPPD129"),
                         "T2T", "Non.T2T")) -> liftoff_merge

library(writexl)

liftoff_merge

liftoff_merge %>% group_by(coding_type) %>% summarise(min(annotation_pct),max(annotation_pct),mean(annotation_pct),median(annotation_pct))
liftoff_merge %>% group_by(cohort,coding_type) %>% summarise(mean(annotation_pct))

head(liftoff_merge)
sampleinfo

liftoff_merge %>% left_join(sampleinfo %>% select(KPPD_ID,sex) %>% rename(sampleID = KPPD_ID)) %>%
  group_by(sex,coding_type) %>%
  summarise(min(annotation_pct),max(annotation_pct),mean(annotation_pct),median(annotation_pct))

liftoff_merge %>% left_join(sampleinfo %>% select(KPPD_ID,sex) %>% rename(sampleID = KPPD_ID)) %>%
  group_by(sex,cohort,coding_type) %>%
  summarise(min(annotation_pct),max(annotation_pct),mean(annotation_pct),median(annotation_pct)) %>% 
  filter(coding_type %in% c("Protein_coding_genes","Protein_coding_trans"))


library("openxlsx")

liftoff_merge %>% select(ID,coding_type,cohort,annotation_pct) %>%
  mutate(ID = str_replace_all(ID,"_",".")) %>%
  rename(Batch = cohort) %>% pivot_wider(names_from = coding_type,values_from = annotation_pct) %>%
  select(Batch,ID,Protein_coding_genes,Noncoding_genes,Protein_coding_trans,Noncoding_trans) %>%
  write.xlsx("/Users/ksmpooh/Desktop/KCDC/pangenome/KPPD/figure/draft_korean_pangenome_collection/Supplement_T4.Completeness_Liftoff.xlsx")

liftoff_merge %>% group_by(cohort,coding_type) %>% summarise(annotation_pct=mean(annotation_pct)) %>%
  rename(Batch = cohort) %>% pivot_wider(names_from = coding_type,values_from = annotation_pct) %>%
  select(Batch,Protein_coding_genes,Noncoding_genes,Protein_coding_trans,Noncoding_trans) %>%
  write.xlsx("/Users/ksmpooh/Desktop/KCDC/pangenome/KPPD/figure/draft_korean_pangenome_collection/Supplement_T4.Completeness_Liftoff_Batch.xlsx")

  


  
#Protein_coding_genes =   n/20089

#Noncoding_genes =   n/57606
#Protein_coding_trans =   n/116,490
#Noncoding_trans =   n/271,454

#### final CNV
setwd("/Users/ksmpooh/Desktop/KCDC/pangenome/KPPD/liftoff/with_flagger/summary")
