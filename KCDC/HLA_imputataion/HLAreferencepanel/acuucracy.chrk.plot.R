#### ¤Àonly not less HLA type accracy
library(stringr)
library(tidyverse)
setwd("~/")
...
################
#setwd("/Volumes/DATA/HLAreferencePanel/Final/accuracy/")
setwd("~/Desktop/KCDC/HLAimputation/MakeReferencePanel/Final/accuracy/")
flist = grep(list.files("./"),pattern = "missINFO", invert=TRUE, value=TRUE)
#flist %>% 
flist <- flist[!grepl("1KGP",flist)]
...
for (i in 1:length(flist)) {
  df <-read.table(flist[i],header = T)
  a <- df %>% summarise(across(colnames(df)[-1],sum))
  for (gene in c("HLA_A","HLA_B","HLA_C","HLA_DRB1","HLA_DQB1")) {
    a[,gene] <- a[,paste0(gene,".match")]/(a[,paste0(gene,".match")] + a[,paste0(gene,".wrong")])
  }
  a <- a %>% mutate("overall" = match_Sum/(match_Sum + wrong_Sum),"filename" = flist[i])
  out <- rbind(out,a)
}
out_all %>% full_join(out) -> out

out %>%
  mutate(filename = str_replace_all(filename,".txt","")) %>% #head()
  mutate('digit' = ifelse(grepl(pattern = "td",filename),"2","4")) %>% #count(digit)
  mutate("CV" = str_split_fixed(filename,"\\.",3)[,2]) %>% #filter(grepl("KBA",filename))
  mutate("CV" = str_split_fixed(CV,"_",3)[,1]) %>% 
  mutate("Ref" = str_split_fixed(filename,"\\.",4)[,4]) %>%
  mutate("Ref" = str_split_fixed(Ref,"_",2)[,1]) %>%
  mutate(Ref = ifelse(grepl("KBA",Ref),"KMHC",Ref)) -> out


setwd("~/Desktop/KCDC/HLAimputation/MakeReferencePanel/test/michigan/03.allele.matching/")
flist = grep(list.files("./"),pattern = "missINFO.txt", invert=TRUE, value=TRUE)
flist = flist[grepl("cmp_Nomencleaner",flist)]
flist
michigan <- NULL
for (i in 1:length(flist)) {
  df <-read.table(flist[i],header = T)
...
michigan %>% mutate('digit' = ifelse(grepl(pattern = "td",filename),"2","4")) %>% #count(digit)
  mutate("CV" = str_split_fixed(filename,"\\.",3)[,2]) %>% #filter(grepl("KBA",filename))
  mutate("CV" = str_split_fixed(CV,"_",3)[,1]) %>% filter(CV != "520sample") -> michigan
...
colnames(out_all)
head(out)
out_all %>% full_join(out) %>% filter(grepl("1KGP",filename))
out %>% filter(grepl("1KGP",filename))


head(out)
table(out$Ref)
### QC accuracy check
out %>% filter(grepl("SNPQC_520sample",filename)) %>% full_join(michigan) %>% #ount(Ref) #%>%
  filter(!grepl("_td.cmp",filename)) %>% mutate(digit= ifelse(digit == 2,"One-field","Two-field")) -> df
#-> df
  
df %>%
  select(HLA_A,HLA_B,HLA_C,HLA_DRB1,HLA_DPA1,HLA_DPB1,HLA_DRB1,HLA_DQA1,HLA_DQB1,overall,CV,Ref,digit) %>% #count(Ref)
  ...
  group_by(Ref,digit,Gene) %>% summarise(Accuracy = mean(Accuracy)) %>%
  ggplot(aes(x=Gene,y=Accuracy,fill=Ref)) +
  geom_bar(position='dodge', stat='identity')  +
  #coord_cartesian(ylim = c(0.7, 1)) +
  facet_grid(~digit)

plot_tehem <- theme(legend.title=element_blank(),
...
df %>% select(HLA_A,HLA_B,HLA_C,HLA_DRB1,HLA_DPA1,HLA_DPB1,HLA_DRB1,HLA_DQA1,HLA_DQB1,overall,CV,Ref,digit) %>% #count(Ref)
  pivot_longer(1:9,names_to = 'Gene',values_to = 'Accuracy') %>%# head()
  mutate(Gene = ifelse(grepl("HLA",Gene),str_split_fixed(Gene,"_",2)[,2],'Overall')) %>% #head()
  filter(Ref != "1KGP") %>% 
  mutate(Ref = factor(Ref, levels = c("KMHC","Multi-ethnic","Han", "PanKor"), labels = c("KMHC", "Multi-ethnic","Han Chinese", "Pan-Kor"))) %>% 
  group_by(Ref,digit) %>% summarise(Accuracy = mean(Accuracy)) %>%  #head()
  ggplot(aes(x=Ref,y=Accuracy,fill=Ref))+
  #geom_boxplot() +
...
## inter 4 ref
setwd("~/Desktop/KCDC/HLAimputation/MakeReferencePanel/Final/v2.allRefCommon/accuracy/")
flist = grep(list.files("./"),pattern = "missINFO", invert=TRUE, value=TRUE)
flist <- flist[!grepl("1KGP",flist)]
out <- NULL
for (i in 1:length(flist)) {
...
out %>%
  ...
  mutate("Ref" = str_split_fixed(filename,"\\.",4)[,4]) %>%
  mutate("Ref" = str_split_fixed(Ref,"_",2)[,1]) %>%
  mutate(Ref = ifelse(grepl("KBA",Ref),"KMHC",Ref)) %>% 
  mutate(Ref = ifelse(grepl("cmp",Ref),"Multi-ethnic",Ref)) %>%
  mutate(digit= ifelse(digit == 2,"One-field","Two-field")) -> df
...
df %>% select(HLA_A,HLA_B,HLA_C,HLA_DRB1,HLA_DPA1,HLA_DPB1,HLA_DRB1,HLA_DQA1,HLA_DQB1,overall,CV,Ref,digit) %>% #count(Ref)
  ...
  filter(Ref != "1KGP") %>% 
  mutate(Ref = factor(Ref, levels = c("KMHC","Multi-ethnic","Han", "PanKor"), labels = c("KMHC", "Multi-ethnic","Han Chinese", "Pan-Kor"))) %>% 
  group_by(Ref,digit) %>% summarise(Accuracy = mean(Accuracy))-> a
  
  
df %>% select(HLA_A,HLA_B,HLA_C,HLA_DRB1,HLA_DPA1,HLA_DPB1,HLA_DRB1,HLA_DQA1,HLA_DQB1,overall,CV,Ref,digit) %>% #count(Ref)
  pivot_longer(1:9,names_to = 'Gene',values_to = 'Accuracy') %>%# head()
  mutate(Gene = ifelse(grepl("HLA",Gene),str_split_fixed(Gene,"_",2)[,2],'Overall')) %>% #head()
  #filter(Ref != "1KGP") %>% #head()
  group_by(Ref,digit,Gene) %>%
  summarise(Accuracy = mean(Accuracy)) %>% #head()
  pivot_wider(names_from = "Gene",values_from = "Accuracy") -> a
  


ori <- readxl::read_xlsx("~/Desktop/KCDC/HLAimputation/MakeReferencePanel/Result/HLAimputation_result_compare.4panels.xlsx")
head(ori)
ori %>% mutate(Ref = panel) %>%
  select(HLA_A,HLA_B,HLA_C,HLA_DRB1,HLA_DPA1,HLA_DPB1,HLA_DRB1,HLA_DQA1,HLA_DQB1,overall,CV,Ref,digit) %>% #head()
  pivot_longer(1:9,names_to = 'Gene',values_to = 'Accuracy') %>%# head()
  mutate(Gene = ifelse(grepl("HLA",Gene),str_split_fixed(Gene,"_",2)[,2],'Overall')) %>% #head()
  filter(Ref != "1KGP") %>% #count(Ref)
  #mutate(Ref = factor(Ref, levels = c("KMHC","Multi-ethnic","Han", "PanKor"), labels = c("KMHC", "Multi-ethnic","Han Chinese", "Pan-Kor"))) %>% 
  group_by(Ref,digit) %>% summarise(Accuracy = mean(Accuracy))-> a
...
#### R2 > 0.8
setwd("~/Desktop/KCDC/HLAimputation/MakeReferencePanel/Final/v3.upper/accuracy/")
flist = grep(list.files("./"),pattern = "missINFO", invert=TRUE, value=TRUE)
flist <- flist[!grepl("1KGP",flist)]
out <- NULL
for (i in 1:length(flist)) {
...
out %>%
  mutate(filename = str_replace_all(filename,".txt","")) %>% #head()
  mutate('digit' = ifelse(grepl(pattern = "td",filename),"2","4")) %>% #count(digit)
  mutate("CV" = str_split_fixed(filename,"\\.",3)[,1]) %>% #filter(grepl("KBA",filename))
  mutate("CV" = str_split_fixed(CV,"_",3)[,1]) %>% 
  mutate("Ref" = str_split_fixed(filename,"\\.",4)[,4]) %>% count(Ref)
  mutate("Ref" = str_split_fixed(Ref,"_",2)[,1]) %>%
  mutate(Ref = ifelse(grepl("KBA",Ref),"KMHC",Ref)) %>% 
  mutate(Ref = ifelse(grepl("cmp",Ref),"Multi-ethnic",Ref)) %>%
  mutate(digit= ifelse(digit == 2,"One-field","Two-field")) -> df
...
out %>% mutate(QC = ifelse(grepl("520sample_SNPQC",filename),"520sample_SNPQC","SNPQC_520sample")) %>% #filter(grepl("1KGP",filename))
  filter(digit == 4) %>% #count(Ref)
  select(HLA_A,HLA_B,HLA_C,HLA_DRB1,HLA_DPA1,HLA_DPB1,HLA_DRB1,HLA_DQA1,HLA_DQB1,overall,CV,Ref,QC) %>% #count(Ref)
