...
### R2 > 0.8 HLA type filter
##bcftools view -e ID==@../KMHC.rare.2field.txt
os.system("bcftools query -f '%%ID\\t%%R2' %s | grep HLA | awk '$2<0.8{print $1}' > %s.R2under0.8.txt"%(vcfIn,out_path))
newVCFin = vcfIn.replace("dose","dose_R2upper0.8")
os.system("bcftools view -e ID==@%s.R2under0.8.txt %s -Oz -o %s"%(out_path,vcfIn,newVCFin))
vcfIn = newVCFin
