...
setwd(paste0("c:/Users/user/Desktop/KCDC/HLAimputation/all/",tool,".",ref,"/"))
digit = "2"

df <- read.csv(paste0("compare.IMPvsNGS.all.gene.",digit,"digit.csv"),header = T)
df<-df[!df$YSample == 'CDC015',]

a <- accuracy.cal(df,paste0(ref,".",tool),digit,check,gene,out.subset)

digit = "4"
df <- read.csv(paste0("compare.IMPvsNGS.all.gene.",digit,"digit.csv"),header = T)
df<-df[!df$YSample == 'CDC015',]

b <- accuracy.cal(df,paste0(ref,".",tool),digit,check,gene,out.subset)
...
write.csv(out,"c:/Users/user/Desktop/KCDC/HLAimputation/all/cookHLAvsIMPUTE4.compare.Result.csv",col.names = T,row.names = F,quote = F)
